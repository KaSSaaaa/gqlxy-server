         ...
         overloaded{
              ...
              [&](const Resolver& nestedResolver) -> Task<nlohmann::json> {
                  ...
                  if (selectionSet) {
                      ...
                      for (const auto& [key, value] : nestedResolver)
                          obj[key] = co_await Resolve(value, {}, nullptr, "", schemaDefinition, fragments, variables);
                  }
                  co_return obj;
              },
              [&](const vector<ValueResolver>& vec) -> Task<nlohmann::json> {
                  ...
                  for (const auto& item : vec)
                      arr.push_back(co_await Resolve(item, {}, selectionSet, typeName, schemaDefinition, fragments, variables));
                  co_return arr;
              },
              [&](const FunctionResolver& func) -> Task<nlohmann::json> {
                  co_return co_await Resolve(func(args), {}, selectionSet, typeName, schemaDefinition, fragments, variables);
              },
              [&](const AsyncFunctionResolver& func) -> Task<nlohmann::json> {
                  co_return co_await Resolve(func(args).get(), {}, selectionSet, typeName, schemaDefinition, fragments, variables);
              },
              [&](const CoroutineResolver& func) -> Task<nlohmann::json> {
                  co_return co_await Resolve(co_await func(args), {}, selectionSet, typeName, schemaDefinition, fragments, variables);
              },
              [&](const CallbackResolver& func) -> Task<nlohmann::json> {
                  promise<ValueResolver> p;
                  func(args, [&p](const auto& res) { p.set_value(res); });
                  co_return co_await Resolve(p.get_future().get(), {}, selectionSet, typeName, schemaDefinition, fragments, variables);
              },
         },
         resolver);
}
    ...
    try {
        ...
        if (document.operations.empty() && args.query.find_first_not_of(" \t\n\r") != string::npos) {
            co_return ResolveResult {
                .errors = nlohmann::json{
                    {"errors", {{{"message", "Failed to parse query"}}}}
                }.dump()
            };
        }
        ...
        for (const auto& op : document.operations) {
            ...
            for (const auto& field : FlattenSelections(op.selectionSet, document.fragments)) {
                ...
                data[outputKey] = co_await Resolve(fieldResolvers.at(field.name),
                                                   ResolverArgs {
